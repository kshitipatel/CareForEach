...
<div class="col-sm-12 w3-agileits-crd widgettable">
	...
	<div class="card card-contact-list">
		<div class="agileinfo-cdr">
			<form class="form-horizontal" name="RegForm" action="{{ route('report.filter',['name' => 'Pending Collection']) }}" method="post" enctype="multipart/form-data">
				{{ csrf_field() }}
				...
				<div class="form-group">
					...
					?>
					<a href="{{ route('report.pendingcollectexport', ['id' => $alreadyselected, 'cdate1' => $cdate1, 'cdate2' => $cdate2 ]) }}"><button type = "button" style="width:15%" class = "btn btn-success"><i class="fa fa-file-excel-o"></i> &nbsp;Export as Excel</button></a>
				</div>
			</form>
			...
			<div class="card-body p-b-20">
				<div class="list-group">
					@foreach($pending_collection as $value)
					<div class="list-group-item media">	
						<div class="media-body">
							<div class="pull-left">	
								<h5><b>Name: </b> {{$value->ename}}</h5>
								<h5><b>Email:</b> {{$value->e_emailid}}  </h5>
								<h5><b>Order Date:</b> {{$value->date}}  </h5>
								<h5><b>Order Id:</b> {{$value->oid}} </h5>
								<h5><b>Grand Total:</b> {{$value->grandtotal}}  </h5>
								<h5><b>Total Paid Amount:</b> {{$value->paidamount}}  </h5>
								<?php $pa = $value->grandtotal - $value->paidamount; ?>
								<h5><b>Total Pending Amount:</b> <?php echo $pa; ?>  </h5>
								<?php $id = $value->oid;?>
								@foreach($remind as $r)
								<?php 
								if($r->oid == $id){
								...
								?>
								@endforeach
								<h5><b>Remind Date:</b> <?php echo $remindd;?>  </h5>
